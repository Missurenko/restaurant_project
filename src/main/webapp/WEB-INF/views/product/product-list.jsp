                ...
                <div class="alert alert-dark">
                    <div class="row">
                        <div class="col-12 col-sm-7 col-md-8">
                            <div> <strong>$  ajsp;djasdap[ {HelloWorld}</strong></div>
            ...
            <div class="row mt-1">
                <div class="col-12 col-sm-3 col-md-4 col-lg-4">
                    <form>
                        ...
                        <div class="form-group">
                            <label for="productSort">Сортировка</label>
                            <select id="productSort" name="productSort" class="form-control">
                                <option value="<%= ProductSort.NAME_ASC.name() %>"
                                        <%= ProductSort.NAME_ASC.name().equals(request.getParameter("productSort")) ? "selected" : "" %>>По
                                    алфавиту А-Я
                                </option>
                                <option value="<%= ProductSort.NAME_DESC.name() %>"
                                        <%= ProductSort.NAME_DESC.name().equals(request.getParameter("productSort")) ? "selected" : "" %>>По
                                    алфавиту Я-А
                                </option>
                                <option value="<%= ProductSort.PRICE_ASC.name() %>"
                                        <%= ProductSort.PRICE_ASC.name().equals(request.getParameter("productSort")) ? "selected" : "" %>>От
                                    дешевых к дорогим
                                </option>
                                <option value="<%= ProductSort.PRICE_DESC.name() %>"
                                        <%= ProductSort.PRICE_DESC.name().equals(request.getParameter("productSort")) ? "selected" : "" %>>От
                                    дорогих к дешевым
                                </option>
                            </select>
                        </div>
